...
export default function MatchPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function changeStatus(newStatus: MatchStatus) {
    ...
    if (newStatus === 'live') { update.home_score = 0; update.away_score = 0 }
  ...
  async function saveResult() {
    if (!match) return
    setSaving(true)
    const { error } = await supabase.from('matches').update({ home_score: homeScore, away_score: awayScore, notes: notes || null }).eq('id', id)
    setSaving(false)
  ...
  return (
    <div className="space-y-6 max-w-lg">
      <div className="flex items-center gap-3">
        <Link href={`/t/${slug}/operator`} className="text-gray-400 hover:text-gray-600 text-sm">← Volver</Link>
      </div>

      <div className="bg-white rounded-xl border p-5 space-y-3">
        <div className="flex items-center gap-2 flex-wrap">
          {match.discipline && <span className={`text-xs px-2 py-0.5 rounded border font-medium ${genderCls}`}>{SPORT_LABELS[match.discipline.name]} {match.discipline.gender}</span>}
          {match.group && <span className="text-xs text-gray-400">{match.group.name}</span>}
          {match.bracket_position && <span className="text-xs font-bold px-2 py-0.5 rounded bg-purple-100 text-purple-700">{match.bracket_position}</span>}
          ...
          {match.scheduled_at && <span className="text-xs text-gray-400">{fmtDateTime(match.scheduled_at)}</span>}
        </div>
        ...
        <div className="flex items-center justify-between gap-4 py-4">
          ...
          <div className="text-center">
            ...
            <div className="mt-1">
              ...
              {match.status === 'finished' && <span className="bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded-full">Final</span>}
              {match.status === 'scheduled' && <span className="bg-blue-100 text-blue-700 text-xs px-2 py-0.5 rounded-full">Programado</span>}
            </div>
          </div>
          <div className="text-center flex-1">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-center gap-3">
                <button onClick={() => setHomeScore((p) => Math.max(0, p - 1))} className="w-10 h-10 rounded-full border-2 border-gray-200 text-xl font-bold hover:border-gray-400 transition disabled:opacity-30" disabled={!isEditable}>−</button>
                <span className="text-4xl font-bold tabular-nums w-12 text-center">{homeScore}</span>
                <button onClick={() => setHomeScore((p) => p + 1)} className="w-10 h-10 rounded-full border-2 border-gray-200 text-xl font-bold hover:border-gray-400 transition disabled:opacity-30" disabled={!isEditable}>+</button>
              </div>
            </div>
            <div className="space-y-2">
              <Label className="text-xs text-gray-500 block text-center">{match.away_team?.name}</Label>
              <div className="flex items-center justify-center gap-3">
                <button onClick={() => setAwayScore((p) => Math.max(0, p - 1))} className="w-10 h-10 rounded-full border-2 border-gray-200 text-xl font-bold hover:border-gray-400 transition disabled:opacity-30" disabled={!isEditable}>−</button>
                <span className="text-4xl font-bold tabular-nums w-12 text-center">{awayScore}</span>
                <button onClick={() => setAwayScore((p) => p + 1)} className="w-10 h-10 rounded-full border-2 border-gray-200 text-xl font-bold hover:border-gray-400 transition disabled:opacity-30" disabled={!isEditable}>+</button>
              </div>
            </div>
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs text-gray-500">Notas (opcional)</Label>
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} disabled={!isEditable} placeholder="Observaciones del partido..." className="w-full text-sm border rounded-md px-3 py-2 resize-none h-16 focus:outline-none focus:ring-2 focus:ring-gray-900 disabled:bg-gray-50 disabled:text-gray-400" />
          </div>
      ...
      {match.status === 'finished' && (
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 text-sm text-gray-600">
          ...
          <button onClick={() => changeStatus('live')} className="text-xs text-blue-600 hover:underline mt-2 block" disabled={changingStatus}>
