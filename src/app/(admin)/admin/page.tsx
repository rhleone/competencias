    ...
    <main className="p-8">
      ...
      {active ? (
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <Link href={`/admin/editions/${active.id}`} className="ml-auto text-sm text-blue-600 hover:underline">
              Gestionar →
            </Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard label="Disciplinas" value={stats.disciplines} icon="🏆" />
            <StatCard label="Inscripciones" value={stats.teams} icon="👕" />
            <StatCard label="Partidos jugados" value={stats.matchesPlayed} icon="✅" />
            <StatCard label="Total de partidos" value={stats.matchesTotal} icon="📅" />
          </div>
      ...
      <div className="bg-white rounded-lg border">
        <div className="p-6 border-b flex items-center justify-between">
          ...
          <Link href="/admin/editions/new" className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition">
            + Nueva Edición
          </Link>
        </div>
        <div className="p-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        {edition.year} · {new Date(edition.start_date + 'T00:00:00').toLocaleDateString('es-AR', { day: 'numeric', month: 'short' })} — {new Date(edition.end_date + 'T00:00:00').toLocaleDateString('es-AR', { day: 'numeric', month: 'short' })}
                      </p>
                    </div>
                  </div>
      ...
      <div className="mt-6 flex gap-3 flex-wrap">
        <Link href="/resultados" target="_blank" className="text-sm text-gray-500 hover:text-gray-700 underline underline-offset-2">
          Ver resultados públicos ↗
        </Link>
        <Link href="/fixture" target="_blank" className="text-sm text-gray-500 hover:text-gray-700 underline underline-offset-2">
          Ver fixture público ↗
        </Link>
      </div>
    </main>
